...
# TODO: Integration in fan mode.
    ...
    #         self.setDriver('CLIFS', utilities.to_isy_fan_mode_value(str(c_mode)))
    ...
    #     try:
    #         LOGGER.debug('process_mode incoming value: ' + str(mode))
    #         daikin_control = Daikin(self.ip)
    #         if int(mode) != 10:
    #             daikin_control.__setattr__('mode', mode)
    #         self.setDriver("GV2", mode)
    #         self.setDriver('CLIMD', utilities.to_isy_mode_value(int(mode)))
    #     except Exception as ex:
    #         LOGGER.exception("Could not refresh diakin sensor %s because %s", self.address, ex)
